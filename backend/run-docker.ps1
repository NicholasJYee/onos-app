...
$AppProjectName = "onos-backend"
$AppContainerName = "onos-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "onos-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server onos-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server onos-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "onos-backend"
        "backend" = "onos-backend"
        "meeting" = "onos-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "onos-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, onos-backend"
        ...
        $logArgs += @("whisper-server", "onos-backend")
    }
