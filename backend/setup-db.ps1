...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.onos\meeting_minutes.db",
        "$env:USERPROFILE\Documents\onos\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/onos-backend/*/backend/meeting_minutes.db"
    }
