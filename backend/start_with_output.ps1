...
Write-Host "====================================="
Write-Host "ONOS Backend Startup"
Write-Host "====================================="
...
Write-Host "====================================="
Write-Host "Starting ONOS Backend"
Write-Host "====================================="
...
# Check if onos-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for onos-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\onos-frontend\onos-frontend.exe",
    "$env:LOCALAPPDATA\Programs\onos\onos-frontend.exe",
    "$env:ProgramFiles\onos-frontend\onos-frontend.exe",
    "${env:ProgramFiles(x86)}\onos-frontend\onos-frontend.exe",
    "$env:APPDATA\onos-frontend\onos-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("onos-frontend.exe", "onos.exe")
...
if ($frontendInstalled) {
    Write-Host "ONOS frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the ONOS frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching ONOS frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "ONOS frontend launched successfully."
        }
    }
} else {
    Write-Host "ONOS frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the ONOS frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if onos is now installed and launch it
