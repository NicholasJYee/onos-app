...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\onos-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("onos-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("onos-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/onos-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("onos-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/onos-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("onos-recordings")
    }
