...
// ============================================================================
// VRAM Detection and GPU Layer Calculation (MOVED FROM main.rs INTO lib.rs)
// ============================================================================
...
/// This is the function your iOS app will call.
...
// ============================================================================
// Non-stdin, in-process API for iOS (and optionally desktop)
// ============================================================================

static GLOBAL_STATE: Lazy<Mutex<anyhow::Result<ModelState>>> = Lazy::new(|| {
    Mutex::new(ModelState::new())
});

fn with_global_state<F, T>(f: F) -> Result<T>
where
    F: FnOnce(&mut ModelState) -> Result<T>,
{
    let guard = GLOBAL_STATE
        .lock()
        .map_err(|_| anyhow::anyhow!("ModelState mutex poisoned"))?;

    // We need to handle the fact initialization can fail.
    // If it failed once, we surface the same error on every call.
    let mut guard = guard;

    let state = guard
        .as_mut()
        .map_err(|e| anyhow::anyhow!("ModelState init failed: {}", e))?;

    f(state)
}

/// Non-stdin generation entrypoint (safe for iOS).
///
/// This is the function you should call from Tauri on iOS.
/// It does not read stdin and does not spawn a subprocess.
///
pub fn generate_once(
    ...
    with_global_state(|state| {
        state.load_model_if_needed(std::path::PathBuf::from(model_path), context_size)?;
        state.generate(
            prompt.to_string(),
            max_tokens,
            temperature,
